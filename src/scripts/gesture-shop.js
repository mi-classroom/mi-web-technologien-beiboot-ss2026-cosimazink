...
// Scroll / navigation
lib.on("pinch-swipe:up",     () => { window.scrollBy({ top: -SCROLL_PX, behavior: "smooth" }); showFeedback("↑", 700); });
lib.on("pinch-swipe:down",   () => { window.scrollBy({ top:  SCROLL_PX, behavior: "smooth" }); showFeedback("↓", 700); });
lib.on("pinch-swipe:left",   () => { history.back();    showFeedback("← zurück", 700); });
lib.on("pinch-swipe:right",  () => { history.forward(); showFeedback("→ vor",    700); });
lib.on("shoulder-tap:left",  () => { history.back();    showFeedback("← zurück", 700); });
lib.on("shoulder-tap:right", () => { history.forward(); showFeedback("→ vor",    700); });
lib.on("hands-to-head:up",   () => { window.scrollTo({ top: 0,                        behavior: "smooth" }); showFeedback("↑↑ oben",  700); });
lib.on("hands-to-hips:down", () => { window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" }); showFeedback("↓↓ unten", 700); });
...
lib.on("shoulder-tap",  ({ state }) => { if (state) showFeedback("○"); });
lib.on("hands-to-head", ({ state }) => { if (state) showFeedback("○"); });
lib.on("hands-to-hips", ({ state }) => { if (state) showFeedback("○"); });
...
lib.on("pinky-pointer:idle", () => { cursorEl.style.display = "none"; });
...
function loop(handRecognizer, poseRecognizer) {
  if (video.readyState < 2 || video.currentTime === lastVideoTime) {
    requestAnimationFrame(loop.bind(null, handRecognizer, poseRecognizer)); return;
  }
  ...
  for (let i = 0; i < handResults.landmarks.length; i++) {
    ...
    drawing.drawLandmarks(lm, {
      color: "#fff", fillColor: label === "Left" ? "#00c8ff" : "#ff6b6b", lineWidth: 1, radius: 3,
    });
  }
  if (poseResults?.landmarks?.length) {
    drawing.drawConnectors(poseResults.landmarks[0], PoseLandmarker.POSE_CONNECTIONS, {
...
function showFeedback(text, clearAfterMs = 0) {
  ...
  if (clearAfterMs > 0) feedbackTimer = setTimeout(() => { feedbackEl.textContent = ""; }, clearAfterMs);
}
function setStatus(text) { statusEl.textContent = text; }

init().catch(err => { setStatus(`Fehler: ${err.message}`); console.error(err); });
