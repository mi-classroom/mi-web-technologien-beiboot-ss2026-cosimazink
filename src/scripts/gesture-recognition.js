...
lib.on("pinch-swipe", ({ state }) => {
  if (state) showLabel(STATE_LABELS[state] ?? state, 300);
});
lib.on("shoulder-tap", ({ state }) => {
  if (state) showLabel(STATE_LABELS[state] ?? state, 300);
});
lib.on("hands-to-head", ({ state }) => {
  if (state) showLabel("○ Hände zum Kopf…", 300);
});
lib.on("hands-to-hips", ({ state }) => {
  if (state) showLabel("○ Hände zur Hüfte…", 300);
});
lib.on("pinky-pointer", ({ state }) => {
  if (state) showLabel(STATE_LABELS.pointing, 300);
});
